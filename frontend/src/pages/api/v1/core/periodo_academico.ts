...
export const POST: APIRoute = async ({ request, redirect, locals }) => {
  ...
  if (!result.success) {
    ...
    return redirect(`/dashboard/configuraciones?tab=periodos&error=${encodeURIComponent(message)}`);
  }
  ...
  try {
    const academicPeriod = await CreateAcademicPeriod(token as string, academicPeriodData);
    if (!academicPeriod) {
      return redirect(`/dashboard/configuraciones?tab=periodos&error=Error al crear el periodo académico`);
    }

    return redirect(`/dashboard/configuraciones?tab=periodos&success=Periodo académico creado correctamente`);
  } catch (error) {
    ...
    return redirect(`/dashboard/configuraciones?tab=periodos&error=${encodeURIComponent(errorMessage)}`);
  }
