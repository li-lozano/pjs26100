...
export const POST: APIRoute = async ({ request, redirect, locals }) => {
  ...
  if (!result.success) {
    ...
    return redirect("/dashboard/matriculas?error=" + encodeURIComponent(msg));
  }
  ...
  const enrollmentData: CreateEnrollmentData = {
    ...
    enrollment_status: result.data.enrollment_status,
  ...
  try {
    ...
    if (!enrollment) {
      return redirect(`/dashboard/matriculas?error=${encodeURIComponent("Error al crear la matrícula")}`);
    }

    return redirect(`/dashboard/matriculas?success=${encodeURIComponent("Matrícula creada correctamente")}`);
  } catch (error) {
    ...
    return redirect(`/dashboard/matriculas?error=${encodeURIComponent(errorMessage)}`);
  }
};