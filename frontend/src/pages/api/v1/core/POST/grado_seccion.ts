...
export const POST: APIRoute = async ({ request, redirect, locals }) => {
  ...
  try {
    const degreeSection = await CreateDegree(
      token as string,
      degreeSectionData,
    );
    if (!degreeSection) {
      return redirect(
        `/dashboard/configuraciones?tab=grados&error=Error al crear la sección del grado`,
      );
    }

    return redirect(
      `/dashboard/configuraciones?tab=grados&success=Sección del grado creada correctamente`,
    );
  } catch (error) {
